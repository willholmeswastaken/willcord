...
const ServerList = () => {
    ...
    useEffect(() => {
        if (server) {
            const serverExists = servers?.some(x => x.server_id === server.id);
            if (!serverExists && servers) {
                navigate(`/${servers![0].server_id}`);
            }
        }
    }, [servers])
    ...
    return (
        <div className='flex flex-col flex-shrink-0 items-center gap-4 py-4 h-screen overflow-y-auto bg-primary'>
            <Link to='/' className='w-12 h-12 rounded-[24px] transition-all hover:rounded-xl bg-indigo-400'>
                <svg xmlns="http://www.w3.org/2000/svg" className="w-10 h-10 pt-[0.4rem] pl-2" viewBox="0 0 71 55" fill="none">
            ...
            <div className="flex flex-col gap-4 bg-primary rounded-full pr-2">
                {servers?.map(serverUser => (
                    <ServerItem key={serverUser.server_id} server={serverUser.Server!} />
                ))}
                <AddServerItem />
            </div>
        </div >
