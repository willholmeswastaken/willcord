...
interface CreateServerInputFormProps {
    name: string;
    image: FileList | null;
}
...
const CreateServerView = ({ onBack }: CreateServerViewProps) => {
    const user = useContext(AuthContext);
    const navigate = useNavigate();
    const setCreateServerModalVisible = useSetAtom(createServerModalVisibleAtom);
    const setCreateServerModalActiveView = useSetAtom(createServerModalViewAtom);
    const defaultServerName = useMemo(() => `${user?.user_metadata.full_name}'s server`, [user?.user_metadata]);
    ...
    const {
        ...
        reset,
        formState: { errors },
    } = useForm<CreateServerInputFormProps>();
            ...
            <form className='relative' onSubmit={handleSubmit(onSubmit)}>
                <div className="flex flex-col p-6">
                    ...
                    <div className='flex justify-center'>
                        <div className="h-[80px] w-[80px] relative">
                            {
                                imagePreview
                                    ? (<img src={imagePreview} alt="serverImagePreview" className='rounded-full w-full h-full' />)
                    ...
                    <label htmlFor='name' className='text-xs text-gray-600 font-semibold'>SERVER NAME</label>
                    <textarea
                ...
                <div className="bg-gray-100 mt-auto ">
                    <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row sm:px-6">
                        <button type="button" className="mt-3 mr-auto inline-flex w-full justify-center py-2 text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:w-auto sm:text-sm" onClick={onBack}>Back</button>
                        <button type="submit" className="inline-flex w-full justify-center rounded-sm border border-transparent bg-indigo-400 px-6 py-2 text-base font-medium text-white shadow-sm hover:bg-indigo-500 sm:ml-3 sm:w-[106.141px] sm:h-[38px] sm:text-sm">
                            {
                                isCreating
                                    ? (<div className='mt-[0.4rem] dot-flashing'></div>)
                                    : 'Create'
                            }
                        </button>
                    </div>
