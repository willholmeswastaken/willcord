...
const CreateServerView = () => {
    ...
    const onSubmit = (data: CreateServerInputFormProps) => {
        console.log(data);

        // upload image first

        // then get url from upload and create server, assign user to server.

        // then redirect to server

        // show a loader inside the create button and disable the form.
    };
    return (
        <>
            <form className='relative' onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="bg-gray-100 mt-auto ">
                    <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row sm:px-6">
                        <button type="button" className="mt-3 mr-auto inline-flex w-full justify-center py-2 text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:w-auto sm:text-sm">Back</button>
                        <button type="submit" className="inline-flex w-full justify-center rounded-sm border border-transparent bg-indigo-400 px-6 py-2 text-base font-medium text-white shadow-sm hover:bg-indigo-500 sm:ml-3 sm:w-auto sm:text-sm">Create</button>
                    </div>
                </div>
